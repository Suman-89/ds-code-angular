...
import { OperatorTypes, ProcessVariableModel, SourceTypes, UIElementTypes } from 'src/app/core/_models';


@Component({
  selector: 'app-shared-ruleset-input-type',
...
bubbleOpt: boolean ;
    ...
    this.emitValue() ;
     ...
     this.rulesetValue = this.singleSelect? 
          this.value && typeof this.value === 'object' ? 
              this.value[0] : this.value ?  this.value : this.variable.value : 
          this.value ? this.value : this.variable.value ;
  }

  emitValue() {
    let obj 
    if(this.variable.valuesource === SourceTypes.REFDATA && this.rulesetValue) {
      obj = this.rulesetValue ;

      this.emitVal.emit(obj) ;
    } else if(this.rulesetValue !== undefined && this.rulesetValue !== null) {
      obj =  typeof this.rulesetValue !== 'object' ? [this.rulesetValue] : this.rulesetValue ;
      this.emitVal.emit(obj) ;
    }
  }
  ...
  setBubbleOption(opt) {
    ...
    if(this.variable.operator === OperatorTypes.EQUALS || this.variable.operator === OperatorTypes.DOES_NOT_EQUALS) {
      this.rulesetValue.includes(opt) ? this.rulesetValue = this.rulesetValue.filter(r => r !== opt) : this.rulesetValue.push(opt) ;
    } else {
