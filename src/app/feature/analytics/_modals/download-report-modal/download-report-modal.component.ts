    ...
    this.repSvc.getReportCondition(this.reportName).subscribe((res) => {
      ...
      this.gridColumnSvc.getTasklistGridColumns(this.type).subscribe((a) => {
        ...
        let columns = [
          ...this.columns,
          ...a.map((element) => {
            let header = element.header ? element.header : '';
            let key = element.key ? element.key : '';
            let max_width = element.width ? element.width : '';
            let filtertype = element.filtertype ? element.filtertype : 'input';
            let cellsformat = element.cellsformat ? element.cellsformat : '';
            return {
              text: header,
        ...
        res.data.Columns = res.data.Columns.filter(
          (r) =>
            r.datafield !== 'productSecondLevel' &&
            r.datafield !== 'partnername' &&
            r.datafield !== 'taskname' &&
            r.datafield !== 'partnerlegalname' &&
            r.datafield !== 'initiatedbyfullname' &&
            r.datafield !== 'assignee' &&
            r.datafield !== 'cename' &&
            r.datafield !== 'contractid' &&
            r.datafield !== 'initiationdate'
        );
