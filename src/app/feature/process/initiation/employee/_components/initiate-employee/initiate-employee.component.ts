...
@Component({
  selector: 'app-initiate-employee',
  templateUrl: './initiate-employee.component.html',
  styleUrls: ['./initiate-employee.component.scss'],
})
export class InitiateEmployeeComponent implements OnInit, OnDestroy {
  @Input() initiation: InitiationTaskModel;
  @Input() form: NgForm;
  @Input() extractedAadhaarObj: any;
  ...
  ngOnInit(): void {
    ...
    this.initiation.product = 'Employee';
    ...
    this.initiation.initiateOfac = false;
    this.initiation.ibasisContractingEntity = 'DSIndia';
    // this.selectedContractingEnt = 'DSIndia';
  }
  ...
  selectCompany(e: TypeAheadModel): void {
    ...
    if (!e.id.includes('create')) {
      ...
      this.initiation.employeeName = e.name;
