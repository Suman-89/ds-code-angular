...
@Component({
  selector: 'app-start-process-aadhaar',
  templateUrl: './start-process-aadhaar.component.html',
  styleUrls: ['./start-process-aadhaar.component.scss'],
})
export class StartProcessAadhaarComponent implements OnInit, OnDestroy {
  ...
  extractedDoc: ExtractedDocModel;
  ...
  saveDocChanges(ev) {
    ...
    console.log(ev);
    // this.extractData();
    this.extractDOCImgData();
    // this.showImage();
  }

  // showImage() {
  //   this.uploadedImg = this.imgEmitter;
  //   // console.log('ami mukul', this.imgEmitter);
  // }
  getImage(input: any) {
    this.uploadedImg = input;
  ...
  closeImg() {
    ...
    this.checkHistoryEmitter.emit(null);
  }

  getHistoryClick(ev) {
    this.checkHistoryClick = ev;
  }

  extractDOCImgData() {
    this.taskSvc.extractDOCImgData(this.formFile).subscribe((p) => {
      console.log(
        'aaaaaaaaaadhaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaar',
        this.formFile
      );

      if (p) {
        let tmpArr = p.data.aadhaarId.trim().split(' ');
  ...
  checkHistory(ev) {
    this.checkHistoryEmitter.emit(ev);
  }
  onOtpEmit(ev) {
    this.otpEmitter.emit(ev);
  }
