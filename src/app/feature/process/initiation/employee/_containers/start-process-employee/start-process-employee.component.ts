...
@Component({
  selector: 'app-start-process-employee',
  templateUrl: './start-process-employee.component.html',
  styleUrls: ['./start-process-employee.component.scss'],
})
export class StartProcessEmployeeComponent implements OnInit, OnDestroy {
  ...
  checkHistoryData: CompanyModel;
  noHistory: boolean;
  companyData: any;
  checkHistoryAadhaarId: string;

  checkHistoryForm: FormGroup = new FormDataConstants().checkHistoryForm;
  ...
  getDocumentList(): void {
    this.taskInfoSvc
      .getEmployeeDocs(this.initiationData.companyCode)
      .subscribe((a) => {
  ...
  formSubmit(): void {
    // this.checkHistoryData.employeeName =
    //   this.checkHistoryData.aadhaar.name;
    this.editCompanyDetails();
    // this.checkHistoryData = this.payloadObj;
    // this.noHistory = true;
    ...
    if (
      ...
      obj.ibasisContractingEntity = 'DSIndia';
      ...
      this.subscription.push(
        this.taskSvc.checkProcess(obj).subscribe((a) => {
          if (a.status) {
            ...
            this.initiationData.ibasisTemplateAvailable = true;
            this.initiationData.requestMsa = false;

            if (a.data.runningInstance) {
  ...
  makeEmployeePrcoessExtraPayload() {
    let payload: any = { aadhaarId: this.checkHistoryAadhaarId };
    if (this.checkHistoryData.employeeName) {
      payload.employeeName = this.checkHistoryData.employeeName;
      payload.partnerLegalName = this.checkHistoryData.employeeName;
    }
    if (this.checkHistoryData.dob) {
      payload.dateOfBirth = this.checkHistoryData.dob;
    }
    if (this.checkHistoryData.aadhaar) {
      if (this.checkHistoryData.aadhaar?.care_of) {
        payload.nameOfFather = this.checkHistoryData.aadhaar?.care_of;
      }
      if (this.checkHistoryData.aadhaar?.gender) {
        payload.gender = this.checkHistoryData.aadhaar?.gender;
      }
      if (this.checkHistoryData.aadhaar?.name) {
        payload.aadhaarName = this.checkHistoryData.aadhaar?.name;
      }
    }
    // "ibasisTemplate"
  ...
  initiateWorkflow(comment?: string): void {
    ...
    obj = { ...obj, ...this.makeEmployeePrcoessExtraPayload() };

    this.initiationData.initiateOfac = false;
    this.subscription.push(
      this.taskSvc
        .startWorkflow({
          ...this.initiationData,
          ...this.makeEmployeePrcoessExtraPayload(),
          contractType: obj.contractType,
          employeeOnboardingChannel: 'Web',
        })
        .subscribe((a) => {
            ...
            this.procFormSvc
              ...
              .subscribe((r) => {
                if (r.data?.key?.variablename) {
                  ...
                  obj.employeeOnboardingChannel = 'Web';
                }
                this.taskSvc
  ...
  aadhaarImgHandler(aadhaarId) {
    if (!aadhaarId) {
      this.clearForm();
    } else {
      this.checkHistory(aadhaarId);
    }
  }
  ...
  checkHistory(aadhaarId) {
    this.companySvc.searchEmployees(aadhaarId).subscribe((res) => {
      if (res.data.dob?.length > 20) {
        res.data.dob = moment(res.data.dob).format('YYYY-MM-DD');
      }
      if (res.data.isNewEmployee) {
        this.toastrSvc.error(
          `Employee dosen't exist in our system! Please fillup the employee details below or click on Verify Aadhaar.`
          //   '', {
          //     positionClass: 'toast-bottom-right'
          //  }
        );
      }
      this.checkHistoryData = res.data;
      this.noHistory = !res.data;

      // console.log(this.checkHistoryData);
      this.checkHistoryAadhaarId = aadhaarId;
      this.signalSvc.viewData.next(this.viewData);
      this.showButton = true;
      this.initiationData.companyCode = aadhaarId;
  ...
  editCompanyDetails() {
    this.checkHistoryData.comptype = 'EMPLOYEE';
    this.checkHistoryData.name = this.checkHistoryAadhaarId;
    if (this.checkHistoryData.employeeName) {
      this.checkHistoryData.partnerLegalName =
        this.checkHistoryData.employeeName;
    }
    // if (this.checkHistoryData.addresses.length === 0) {
    //   this.checkHistoryData.addresses.push({
    //     id: 353,
    //     line: this.checkHistoryData.aadhaar.address,
    //     country: 'India',
    //     countrycode: 'IND',
    //     state: '',
    //     city: '',
    //     postalcode: '',
    //     primary: false,
    //     friendlyaddress: 'India',
    //     type: 'PA',
    //   });
    // }
    this.companySvc
      .editCompany(
        { resource: this.checkHistoryData },
        this.checkHistoryData.id
      )
      .subscribe((a) => {
        if (a.status) {
          console.log('SAVED DATAA ', a);
        }
      });
  }
  ...
  onOtpEmit(otpObj) {
    this.companySvc
      .verifyAadhaar(this.checkHistoryAadhaarId, otpObj.otp)
      .subscribe((res) => {
        if (res.data) {
          otpObj.closeModal();
          if (res.data.dob?.length > 20) {
          ...
          this.toastrSvc.success('Aadhaar verified successfully');
