...
@Component({
  selector: 'app-start-snow-deal',
  templateUrl: './start-snow-deal.component.html',
  styleUrls: ['./start-snow-deal.component.scss'],
})
export class StartSnowDealComponent implements OnInit, OnDestroy {
      ...
      if (i.status) {
        ...
        this.taskInfoSvc.getTaskVariables(i.data[0]).subscribe((v) => {
          if (v.status) {
            ...
            console.log("invExtractinstance?.model?.partnerLegalName",this.invExtractinstance,this.initData)
          }
        });
      }
    });
  ...
  clearCompany(e: boolean): void {
    this.invExtractinstance.model.partnerLegalName = '';
  }
  ...
  //   this.initiationData.next(this.initiation);
  //   this.initToDetailsEmitter.emit(event);
  // }
    ...
    this.taskSvc.startWorkflow(m).subscribe((d) => {
      if (d.status) {
        ...
        this.procFormSvc.getProcDefbyName(m.processname).subscribe((r) => {
          ...
          this.toastrSvc.success('Snow Deal Initiated');
          this.back();
        });
      }
    });
