...
@Component({
  selector: 'app-start-voice-deal',
  templateUrl: './start-voice-deal.component.html',
  styleUrls: ['./start-voice-deal.component.scss'],
})
export class StartVoiceDealComponent implements OnInit, OnDestroy {
  ...
  selectCompany(e: TypeAheadModel): void {
    ...
    if (!e.id.includes('create')) {
      this.invExtractinstance.model.dealCompany = e.name;
    ...
    this.taskSvc.startWorkflow(m).subscribe((d) => {
      if (d.status) {
        ...
        this.procFormSvc.getProcDefbyName(m.processname).subscribe((r) => {
          ...
          this.toastrSvc.success('Voice Deal Initiated');
          this.back();
        });
      }
    });
